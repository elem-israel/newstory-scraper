...
from newstory_scraper.util import extract_profile
from newstory_scraper.util import extract_tags
from newstory_scraper.util import get_username_by_id
...
def test_extract_posts_from_tag_scrape(tags):
    extracted = extract_posts(tags)
    assert isinstance(extracted[0]["instagram_post_id"], str)
    assert isinstance(extracted[0]["caption"], str)
    assert isinstance(extracted[0]["taken_at"], datetime)
    assert extracted[0]["instagram_author_profile_id"] == "34392249671"
    assert len(extracted) == 100


def test_extract_hashtags(profile):
    extracted = extract_tags(profile)
    for t in extracted:
